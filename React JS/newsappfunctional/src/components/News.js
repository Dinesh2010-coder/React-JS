...
const News = (props) => {
  ...
  const fetchMoreData = async () => {
    ...
    return (
      <>
        <h1 className='text-center' style={{ margin: '40px 0px', marginTop: '90px' }}>News Monkey -- Top Headines from {capitalizeFirstLetter(props.category)}</h1>
        {loading && <Spinner/>}
        <InfiniteScroll
        ...
        {/* <div className="container d-flex justify-content-between">
          <button disabled={page<=1} className="btn btn-primary" onClick={handlePrevClick}>&larr; Prev</button>
